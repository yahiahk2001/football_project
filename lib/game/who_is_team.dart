...
class _GameScreenState extends State<GameScreen>
  ...
  bool isRoundComplete() {
    return selectedTeams[currentRound]
        .players
        .every((player) => player.isGuessed);
  }
        ...
        if (!player.isGuessed) {
          ...
          for (var acceptableAnswer in player.acceptableAnswers) {
            final distance = calculateLevenshteinDistance(
                answer, acceptableAnswer.toLowerCase());
  ...
  void provideHint() {
    ...
    setState(() {
      showHint = true;
      final unguessedPlayer = selectedTeams[currentRound]
          .players
  ...
  void endRound() {
    ...
    final unguessedPlayers = selectedTeams[currentRound]
        .players
        .where((player) => !player.isGuessed)
        .toList();

    if (currentRound < 2) {
      // Changed to < 2 since we're using 3 rounds (0-2)
      showDialog(
        context: context,
        barrierDismissible: false,
        builder: (context) => WillPopScope(
          onWillPop: () async =>
              false, // Prevent dialog dismissal with back button
          child: AlertDialog(
  ...
  void showGameOverDialog(List<Player> finalUnguessedPlayers) {
    int accuracyBonus =
        (totalCorrectGuesses / (totalCorrectGuesses + totalWrongGuesses) * 50)
            .round();
              ...
              onPressed: () {
                showDialog(
                  ...
                  builder: (context) => AlertDialog(
                    ...
                    content:
                        const Text("هل أنت متأكد أنك تريد الخروج من اللعبة؟"),
                    actions: [
                      TextButton(
